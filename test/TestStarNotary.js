...
it("can add the star name and star symbol properly", async () => {
  ...
  assert.equal(await instance.name.call(), "Star Notary Service");
  assert.equal(await instance.symbol.call(), "SNS");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  let starId1 = 7;
  let starId2 = 8;
  await instance.createStar("First Star", starId1, { from: accounts[0] });
  await instance.createStar("Second Star", starId2, { from: accounts[1] });
...
it("lets a user transfer a star", async () => {
  ...
  let instance = await StarNotary.deployed();
  let starId = 9;
  await instance.createStar("My Star", starId, { from: accounts[0] });
